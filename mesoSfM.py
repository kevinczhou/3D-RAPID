...
class mesoSfM:
    ...
    def _add_regularization_loss(self, reg_coefs):
        ...
        if 'local_illum_flat' in reg_coefs and reg_coefs['local_illum_flat'] is not None:
            # should use this when optimizing `illum_flat`;
            # should only be used if the target object is ideally uniform;
            im_corrected = tf.reshape(self.im, (self.num_images, -1, self.num_channels))
            brightness_variation = tf.math.reduce_std(im_corrected, axis=1)
            brightness_variation = tf.reduce_mean(brightness_variation)
            self.loss_list.append(reg_coefs['local_illum_flat'] * brightness_variation)
